...
# Define model
XGBoostReg = XGBRegressor(tree_method="hist")
...
_print(XGBoostReg_grid_search.best_params_)
...
evalplot.savefig(results_dir + 'histXGBoost_training.png')

# Compute model metrics
met, params, errorplot = model_metrics(XGBoostRegBest, X_test, y_test, scaler = aooScaler)
with open(results_dir + 'histXGBoost_metrics.txt', 'w') as file:
    file.write(met)
    file.write(params)
errorplot.savefig(results_dir + 'histXGBoost_prediction.png')

# Pickle best estimator
with open(results_dir + 'regressors/histXGBoost_regressor.pkl', 'wb') as f:
